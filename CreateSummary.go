...
func makeoutfile(){
	...
	Outstring = joinstrings(Outstring, runcmdother("find /etc -type f -mtime 0"), "Modified files in /etc in past day")
	fmt.Println("Finding most recently modified files... \n")
	Outstring = joinstrings(Outstring, runcmdother("find ~/.ssh -type f -mtime 0"), "Modified files in ~/.ssh in past day")
